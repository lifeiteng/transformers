...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, seq_len: int = 0):
        ...
        #     self._set_cos_sin_cache(seq_len=seq_len.max().item(), device=x.device, dtype=x.dtype)
...
def apply_rotary_pos_emb(q: torch.Tensor, k: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, position_ids: Optional[torch.LongTensor], unsqueeze_dim: int =1):
...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        assert not (self.pretraining_tp > 1)

    def forward(self, x):
        if not torch.jit.is_scripting() and self.pretraining_tp > 1:
...
class LlamaDecoderLayer(nn.Module):
    ...
    ) -> Tuple[torch.FloatTensor, Optional[Union[Tuple[torch.FloatTensor, Cache], Cache]]]:
        """
        Args:
